...
def find_latest_results(task_name="ner"):
    ...
    # Go up one level to find results
    base_dir = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
    task_dir = os.path.join(base_dir, 'results', task_name)
...
def convert_to_gguf_f16(merged_model_path, gguf_output):
    ...
    subprocess.run(
        [sys.executable, "-m", "pip", "install", "-q", "-r", os.path.join(llama_cpp_dir, "requirements.txt")],
        check=False
    )
...
def main():
    ...
    base_dir = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
    output_dir = os.path.join(base_dir, "best_model", args.task)
    ...
    logger.info("="*80)
    logger.info("\n1. Test the model:")
    logger.info(f"   uv run scripts/inference_gguf.py --model {gguf_f16_path} --test")
